...
normal folder named something like `fantasy-league-almanac-2.2.0`. Work
...
1. Open the extracted `fantasy-league-almanac-2.2.0` folder in File Explorer, so its contents -- including `QUICKSTART.md` and `requirements.txt` -- are displayed.
...
Your prompt should look roughly like `PS C:\Users\YourName\...\fantasy-league-almanac-2.2.0>`. The exact folders before the almanac name will differ, and that is fine.
