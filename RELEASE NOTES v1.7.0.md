...
A career Hall of Shame split by production type, and the futility block that both books now share, with the two Halls in lockstep across platforms, so the same column means the same thing on either side. They are the first records surfaces to explicitly converge; the rest of the records pages remain largely platform-specific for now. On the records tab; shows all-time contribution by player/team combo, plus the players whose MLB careers have been put to the worst use by your league.
