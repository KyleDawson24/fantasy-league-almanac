# v1.5.0 -- rebuilt against the commit range

**Review copy.** Baseline = origin's `v1.2.0` (`1eb821e0`, 2026-05-31).
Range = 226 commits → 158 substantive, 68 bookkeeping (roadmap mirrors,
handoffs, ledger entries, catalog refreshes). Every substantive commit
below is accounted for: mapped to an existing draft bullet, folded into
one, or written fresh.

Items marked **[NEW]** did not exist in `RELEASE NOTES v1.5.0 DRAFT.md`
*or* in the CHANGELOG's `[Unreleased]` -- they are the work that shipped
without ever being written down. Strip the markers at commit time.
...
- **[NEW] Dev/prod Sheet targeting.** Every Sheets surface takes an
...
- **[NEW] `dim_player_identity` -- one identity resolver, id-first joins
...
- **[NEW] Franchise and owner continuity -- a 25-year league told as
...
- **[NEW] Baseball Reference links throughout the almanac.** Standalone
...
- **[NEW] Advanced Standings (the tab itself).** A new almanac tab
...
- **[NEW] Advanced Standings round two, both books.** Thirteen live
...
- **[NEW] The canonical stat catalog.** One platform-neutral stat
...
- **Universal MLB stats layer.** CBS's own stats history turned out to
  ...
  - **[NEW] The 2022–23 pitcher note.** Calculated points run ~10–15%
...
- **The CBS almanac.** `output/cbs_almanac_sheets.py` mirrors the ESPN
  ...
  - **[NEW] The Records page, in full.** Auto-cataloged from the stat
    ...
    **Wasted Hall of Shame** ranked by points left on the bench and
    ...
    per-lineage owners.
  - **[NEW] The Home page, in full.** Five review rounds added a Team of
...
- **[NEW] Draft Recap, both books.** CBS gains the tab, mirroring the
...
- **[NEW] The CBS draft data pipeline.** Every CBS draft surface swept
...
- **[NEW] The Trades tab (ESPN).** The live trade block plus per-team
...
- **[NEW] Recap: all-time records set in the most recent week are called
...
- **[NEW] Public-readiness guard rails.** The five owner-identity seeds
...
- **[NEW] Platform adapter contract v1.** The written contract a second
...
- **[NEW] The weekly recap moved to the calculated points lens.** Every
...
- **[NEW] New-record presentation in the recap.** Records broken before
...
- **[NEW] Team-name introduction in recap prose.** A team is introduced
...
- **[NEW] Team totals were rounded per component, not once.** Calculated
...
- **[NEW] Two-way production misbucketed between hitting and pitching.**
...
- **[NEW] Optimal-team attribution for multi-team players.** A player
...
- **[NEW] Google OAuth token expiry killed the run.** An expired Sheets
  token now triggers a graceful re-consent instead of an unhandled
  failure mid-render.

- **[NEW] Sheets per-minute write quota.** The almanac writers survive
  the per-minute quota ceiling, adopting the backoff the ESPN writer
  already used.

- **[NEW] The recap golden depended on gitignored local state.** The
...
- **[NEW] Logged drops didn't close roster stints.** The stint-pairing
...
- **[NEW] Season-report rate stats.** Rate statistics were computed on
...
Bookkeeping with no reader value: roadmap mirrors, session handoffs,
ledger/progress entries, dbt catalog refreshes, `[Unreleased]` updates,
golden re-anchor commits, test-hygiene commits, seed data additions
(nicknames), in-flight worklists (Track B), and the internal v2.0
groundwork brief. Also omitted: minor cosmetic passes folded into the
sections they belong to (column widths, header wrap, alignment, buffer
columns, powder-blue scope headers), and the callout template variants.

## How the gap happened (review note, does not ship)

`[Unreleased]` was written incrementally by sessions mirroring their own
work, so it records what each session remembered to log. The draft was
then curated *from* `[Unreleased]` -- faithfully, which means it
inherited every blind spot. The commit range is the only complete
record. In the shipped section this needs no explaining: 1.3.x/1.4.x
were internal milestones not worth publicizing, and the one-line note in
the intro covers it.

Two open questions the draft raised are answered by the range:

- *"the player identity dimension: no `dim_player` entry exists"* -- it
  exists. `dim_player_identity` shipped 2026-07-14 (`5a81e14`).
- *"Latent since the v1.2 bref-links pass"* -- the bref-links pass is
  2026-06-29 (`5b1c941`…`e03bafb`), **after** v1.2.0. It belongs to this
  release, and the Fixed bullet had referenced a feature the notes never
  introduced. Now corrected.

## Decisions (Per Offline Chat 2026-07-21)

1. **"Box Score Baseball" stays**, wherever it naturally occurs. It's
   the league's actual name and generic enough to be public; `cbs-bsb`
   is the internal technical key.
2. **Adapter contract v1** -- the short bullet stands as written; the
   nuts-and-bolts specs are still settling, so it isn't oversold.
3. **The anonymization bullet stays.**
4. **Callout template variants stay out.** Callouts are planned as
   league-specific rather than platform-specific, so the variants aren't
   part of this release's story.
