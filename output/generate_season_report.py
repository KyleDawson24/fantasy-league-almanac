...
def get_season_player_totals(season_year):
    ...
    *_pts columns), aggregated in Python at (team_id, player_id) grain
    ...
    entity -- same identity the lines print ("Player (ABBR)").
    """
...
def get_biggest_hero(season_year):
    ...
    (the lens that decides W/L), abnormal weeks excluded."""
    rows = query_snowflake("""
        WITH wins AS (
...
def get_team_wasted_totals(season_year):
    ...
    weekly Top Wasted list uses. FA-pool waste belongs to no roster and
    is deliberately absent here (it shows on the player list instead)."""
    return query_snowflake("""
        WITH bench AS (
...
def get_season_wasted_players(season_year, limit=5):
    """Season-total Top Wasted Performances -- the weekly recap query's
    aggregation without the matchup_period pin. Row keys match
    generate_summary.format_wasted_points exactly so the formatter is
    reused verbatim (breakdown parenthetical included)."""
    return query_snowflake("""
        WITH wasted_combined AS (
            SELECT player_id,
                   MAX(player_name) AS display_name,
                   SUM(CASE WHEN wasted_bucket = 'FA'
                            THEN calculated_points ELSE 0 END) AS fa_wasted_pts,
                   SUM(CASE WHEN wasted_bucket = 'ROSTERED_INACTIVE'
                            THEN calculated_points ELSE 0 END) AS bench_wasted_pts,
        ...
        SELECT COALESCE(m.display_name, w.display_name) AS display_name,
               m.pro_team,
               m.position,
               m.eligible_slots,
               ...
               w.fa_wasted_pts,
               w.bench_wasted_pts,
               COALESCE(a.negative_active_pts, 0) AS doubly_wasted_pts,
               w.wasted_points_total + COALESCE(a.negative_active_pts, 0)
                   AS wasted_points,
               a.active_points
        FROM wasted_combined w
        LEFT JOIN player_meta m USING (player_id)
        LEFT JOIN active a USING (player_id)
...
def _fmt_players(player_list, score_key='calculated_points'):
    return ", ".join(
        f"{p['display_name']}: {p[score_key]:.1f}" for p in player_list
    )
...
def format_team_callouts(standings, players):
    ...
    pts/wk) / hitting / pitching with top-3 season contributors, then the
    worsts without contributors. Ranked by per-week production (every
    team shares the denominator, so the order matches season totals)."""
    ...
    return [
        ...
        _fmt_players(_team_top_players(players, best['team_id'], 'calculated_points')),
...
def format_superlatives(season_records, season_player_records, hero,
    ...
    then the matchup-shaped oddities."""
...
def format_all_league_team(rows):
    """Season-to-date All-League Team: Slot: Player (ABBR) -- pts --
    stat line (slash line). Rows arrive canonical-slot-ordered and
    stat-enriched from the optimal-team dispatcher (the same rows the
    almanac Home tab renders)."""
    lines = [
        "",
    ...
    for row in rows:
        ...
        tail = f" ({slash})" if slash and slash != '//' else ""
        lines.append(
            f"[b]{slot}[/b]: {row.get('display_name')} "
            f"({row.get('team_abbrev') or row.get('team_name') or 'FA'}) -- "
            f"{pts:.1f} pts -- {stat_line}{tail}"
        )
    return lines


def format_records_set_this_season(all_time_rows, season_year, schedule_lookup):
    """All-time records currently held by performances from this season.
    Rank-1 recency tiebreaks mean a this-season TIE of an older mark also
    surfaces -- the header says "set or tied" on purpose rather than
    re-running per-week set-vs-tied detection across the whole half.
    Floor-zero rows are skipped (a 0 'record' is nobody's achievement)."""
    polarity = stat_catalog.get_polarity_map()
    auto_tracked = stat_catalog.get_auto_tracked()

    holders = [
        r for r in all_time_rows
        if r['season_year'] == season_year
        and records.should_track_record(
            r['entity_grain'], r['stat_name'], r['record_direction'],
            polarity, auto_tracked,
        )
        and not (r['stat_name'] not in records.SCORE_STAT_NAMES
                 and (r['stat_value'] or 0) == 0)
    ]
    if not holders:
        return []

    lines = ["", "[u][b]All-Time Records Set or Tied This Season[/b][/u]"]
    for r in holders:
        label = _recap_record_label(
            r['entity_grain'], r['stat_name'], r['record_direction'])
        value = fmt_stat_value_with_unit(r['stat_name'], r['stat_value'])
        week = records.format_week_label(
            r['season_year'], r['matchup_period'], schedule_lookup)
        if r['entity_grain'] == 'player':
            holder = f"{r['display_name']} ({r['team_abbrev']}, {r['owner_name']})"
        else:
            holder = f"{r['team_name']} ({r['owner_name']})"
        lines.append(f"[b]{label}[/b]: {holder} -- {value}, {week}")
    return lines


def format_wasted_section(wasted_players, team_totals):
    """Season Top Wasted Performances (the weekly formatter reused over
    season-total rows) plus a one-line per-team wasted ranking."""
    lines = format_wasted_points(wasted_players)
    if lines:
        # The shared formatter's header says "Top N Wasted Performances";
        # retitle for season scope without forking the row rendering.
        lines[1] = f"[u][b]Top {len(wasted_players)} Wasted Performances (Season)[/b][/u]"
    if team_totals:
        ranking = ", ".join(
...
def generate_season_report(season_year):
    ...
    lines = [
        ...
        *format_records_set_this_season(
            all_time_rows, season_year, schedule_lookup),
        *format_wasted_section(
