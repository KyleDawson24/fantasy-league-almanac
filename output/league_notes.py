...
def hero(ctx):
    ...
    hist_n = records.query_snowflake(f"""
        ...
        ranked AS (
          ...
          WHERE {records.league_predicate('p')}
        ),
        top2 AS (
...
def scapegoat_net_negative(ctx):
    ...
    hist_n = records.query_snowflake(f"""
        WITH p AS (
          ...
          WHERE {records.league_predicate()}
          GROUP BY 1, 2, 3
        ),
        t AS (
          SELECT t.season_year, t.matchup_period, t.team_id,
    ...
    hist = records.query_snowflake(f"""
        WITH team_mp AS (
          SELECT
            ...
            d.matchup_period,
            d.team_id,
            CAST(SUM(CAST(CASE WHEN d.platform_points < 0 THEN 1 ELSE 0 END AS DECIMAL(18, 6))) AS DOUBLE) AS neg_days,
